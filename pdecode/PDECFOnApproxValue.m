function [ evivec, dsvec ] = PDECFOnApproxValue( wvec, s0, scale )
% FIX: TO DO: UPDATE WITH ONLINE VERSION OF THIS!
%PDECFOnKGs: Find the best on can do in terms of maximizing expected reward
...
elseif nargin < 3   % no parameter passed in for scale: try to flood
    ...
    for i=1:length(sincrem)           % check the lookaheads over that interval
        evitmp = -(1/svec(i) - 1/s0) + (1/svec(i) - 1/s0)*wvec*alpha? + sqrt(sincrem(i)) * PsiNorm(-wvec/sqrt(sincrem(i)));
...
%    figure(1001); hold off; figure(1002); hold off; figure(1003); hold off;
    for i=1:length(sincrem)           % check the lookaheads over that interval
        evitmp = -(1./(s0-sincrem(i)) - 1/s0) + (1/svec(i) - 1/s0)*wvec*alpha? + sqrt(sincrem(i)) .* PsiNorm(-wvec/sqrt(sincrem(i)));
        dsvec(evitmp > evivec) = sincrem(i) ;
        evivec = max(evivec,evitmp);
