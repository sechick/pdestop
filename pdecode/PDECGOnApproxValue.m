function [ evivec, dsvec ] = PDECGOnApproxValue( wvec, s0, scale )
...
elseif nargin < 3   % no parameter passed in for scale: try to flood
    ...
    for i=1:length(sincrem)           % check the lookaheads over that interval
        ...
        evitmp = (1/svec(i) - 1/s0)*wvec*alpha? + exp(1/s0 - 1/(s0-sincrem(i))) * sqrt(sincrem(i)) * PsiNorm(-wvec/sqrt(sincrem(i)));
